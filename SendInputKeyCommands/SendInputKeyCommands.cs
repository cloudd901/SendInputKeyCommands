...
namespace SendInputKeyCommands
{
    public class SendInputKeyCommand
    {
        /// <summary>
        ...
        {
            bool shift = RequireShift(keyString);
            if (shift) { SendKeyDown(VirtualKeyCode.SHIFT); }
            SendKeyPress(StringToVKC(keyString));
            if (shift) { SendKeyUp(VirtualKeyCode.SHIFT); }
        }
        ...
        {
            if (RequireShift(keyString)) { SendKeyDown(VirtualKeyCode.SHIFT); }
            SendKeyDown(StringToVKC(keyString));
        }
        ...
        {
            SendKeyUp(StringToVKC(keyString));
            if (RequireShift(keyString)) { SendKeyUp(VirtualKeyCode.SHIFT); }
        }
        ...
        private bool RequireShift(string s)
        {
            bool required = false;
            if (s.Contains("{") && s.Contains("}")) { s = s.Replace("{", "").Replace("}", ""); }
            if (s.Length == 1 && char.IsUpper(s.ToCharArray()[0]) && !char.IsDigit(s.ToCharArray()[0])) { required = true; }
        ...
        {
            VirtualKeyCode vkc = VirtualKeyCode.DEFAULTNONE;
            s = s.Replace("{", "").Replace("}", "").ToUpper();
            try
            {
                vkc = (VirtualKeyCode)Enum.Parse(typeof(VirtualKeyCode), "KEY_" + s, false);
            }
            catch
            {
                string newkey = "";
                if (s == " ") { newkey = "SPACE"; }
                else if (s == "." || s == ">") { newkey = "OEM_PERIOD"; }
                else if (s == "ENTER") { newkey = "RETURN"; }
                else if (s == "CTRL") { newkey = "CONTROL"; }
                else if (s == "INS") { newkey = "INSERT"; }
                else if (s == "PRT") { newkey = "PRINT"; }
                else if (s == "PLUS") { newkey = "ADD"; }
                else if (s == "+" || s == "=") { newkey = "OEM_PLUS"; }
                else if (s == "MINUS") { newkey = "SUBTRACT"; }
                else if (s == "-" || s == "_") { newkey = "OEM_MINUS"; }
                else if (s == "WIN") { newkey = "LWIN"; }
                else if (s == "," || s == "<") { newkey = "OEM_COMMA"; }
                else if (s == ";" || s == ":") { newkey = "OEM_1"; }
                else if (s == "/" || s == "?") { newkey = "OEM_2"; }
                else if (s == "`" || s == "~") { newkey = "OEM_3"; }
                else if (s == "[" || s == "{") { newkey = "OEM_4"; }
                else if (s == "\\" || s == "|") { newkey = "OEM_5"; }
                else if (s == "]" || s == "}") { newkey = "OEM_6"; }
                else if (s == "'" || s == "\"") { newkey = "OEM_7"; }

                else if (s == "!") { newkey = "KEY_1"; }
                else if (s == "@") { newkey = "KEY_2"; }
                else if (s == "#") { newkey = "KEY_3"; }
                else if (s == "$") { newkey = "KEY_4"; }
                else if (s == "%" || s == "PERCENT") { newkey = "KEY_5"; }
                else if (s == "^") { newkey = "KEY_6"; }
                else if (s == "&") { newkey = "KEY_7"; }
                else if (s == "*") { newkey = "KEY_8"; }
                else if (s == "(") { newkey = "KEY_9"; }
                else if (s == ")") { newkey = "KEY_0"; }

                else { newkey = s; }
                try
                {
                    vkc = (VirtualKeyCode)Enum.Parse(typeof(VirtualKeyCode), newkey, false);
                }
                catch
                {
